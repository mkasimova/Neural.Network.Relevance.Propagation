...
# print(samples.shape, labels.shape, feature_importance.shape)
...
def scatterplot():
    ...
    for i in range(len(methods)):
        ...
        # Find the two most important contact pairs
        indices = feature_importance[:,0].argsort(axis=0)[[-1, -2]] # Get the indices of the 2 most important features (inputs)
        print(indices)

        resids1 = feature_to_resids[indices[0]].astype(int)# Get the residue ids from top 1 contact pair
        resids2 = feature_to_resids[indices[1]].astype(int)# Get the residue ids from top 2 contact pair
        print(resids1, resids2)

        # Get the residue names from the indices
        contacts1 = [topology.residue(resids1[0]), topology.residue(resids1[1])]
        contacts2 = [topology.residue(resids2[0]), topology.residue(resids2[1])]


        # Get the feature values from all frames for the top 1 and top 2 contacts
        holo_feature1 = samples[labels==1, indices[0]]  # Top 1 across all holo frames
        holo_feature2 = samples[labels==1, indices[1]]  # Top 2 across all holo frames

        apo_feature1 = samples[labels==2, indices [0]]  # Top 1 across all apo frames
        apo_feature2 = samples[labels==2, indices [1]]  # Top 2 across all apo frames
