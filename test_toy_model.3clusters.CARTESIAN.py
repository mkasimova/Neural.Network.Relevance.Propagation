...
def run_all_feature_extractors(data,cluster_indices,n_splits,n_iterations,moved_atoms,feature_to_resids):
        ...
        for k in range(n_iter_per_example):
            ...
            if j%2==1:
                ...
                dg = DataGenerator(natoms=100, nclusters=3, natoms_per_cluster=[10,10,10], nframes_per_cluster=1200, test_model=i_model,\
                               ...
                               displacement=0.1, feature_type='cartesian_rot')
            ...
            projection[:,i,j,k,:], class_score[:,i,j,k,:] = run_all_feature_extractors(data,cluster_indices,n_splits,n_iterations,dg.moved_atoms,feature_to_resids)

work_dir = '/media/mkasimova/Data2/NN_VSD/toy_model/testing.different.toy.models/Febr.12.updated.code/'

np.save(work_dir+'projection.CARTESIAN.NON-LINEAR-P-DISPLACEMENT.npy',projection)
np.save(work_dir+'class_score.CARTESIAN.NON-LINEAR-P-DISPLACEMENT.npy',class_score)
